...
export default function ProductDirectLinkPage({ params }: { params: Promise<{ handle: string, productCode: string }> }) {
  const resolvedParams = use(params);
  const { handle, productCode } = resolvedParams;
  ...
  // Find the requested product to preview
  const targetProduct = store.products.find(p => p.code === productCode || p.id === productCode);
      ...
      <main className="max-w-6xl mx-auto px-4 py-8">
        ...
        {filteredProducts.length === 0 ? (
          ...
          <ProductGrid 
            ...
            initialPreviewProduct={targetProduct}
