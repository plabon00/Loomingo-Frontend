...
export default function InvoiceGenerator() {
  ...
  const updateLineItem = (index: number, field: string, value: string | number) => {
    ...
    if (field === "type") {
      if (value === "Reels") newItems[index].name = "Instagram Reel Video Content & Promotion";
      else if (value === "Story") newItems[index].name = "Instagram Story Content & Promotion";
      else if (value === "Reel + Story") newItems[index].name = "Instagram Reel + Story Campaign Package";
      else if (value === "Product") newItems[index].name = "";
    }
          ...
          <div className="max-w-xl mx-auto space-y-6 pb-20">
            ...
            <Card className="shadow-sm border-zinc-200/60 bg-white overflow-hidden">
              ...
              <CardContent className="p-0">
                {/* Desktop Header Row */}
                <div className="hidden sm:flex justify-between items-center px-6 py-2.5 bg-white border-b border-zinc-100 text-[10px] font-semibold text-zinc-400 uppercase tracking-widest">
                ...
                <div className="flex flex-col divide-y divide-zinc-100">
                  {lineItems.map((item, index) => (
                    <div key={index} className="group flex flex-col gap-3 px-4 sm:px-6 py-4 items-start hover:bg-zinc-50/50 transition-colors">
                      
                      {/* Top Row: Type & Amount */}
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4">
                        {/* Type Dropdown */}
                        <div className="relative border border-zinc-200/60 rounded-full bg-white hover:border-zinc-300 transition-colors shadow-sm shrink-0">
                          <select 
                            className="h-7 text-xs font-semibold text-zinc-700 bg-transparent border-none pl-3 pr-7 outline-none focus:ring-0 cursor-pointer appearance-none"
                            value={item.type}
                        ...
                        {/* Amount */}
                        <div className="flex items-center justify-center gap-1.5 w-full sm:w-40 border border-zinc-200/60 rounded-md bg-white px-3 py-0.5 hover:border-zinc-300 transition-colors shadow-sm focus-within:border-zinc-300 focus-within:ring-2 focus-within:ring-indigo-500/10 sm:mr-4">
                          ...
                          <Input 
                            ...
                            className="w-full text-center bg-transparent border-none px-0 text-lg font-bold text-zinc-900 shadow-none outline-none focus-visible:ring-0 [-moz-appearance:_textfield] [&::-webkit-inner-spin-button]:m-0 [&::-webkit-inner-spin-button]:appearance-none placeholder:font-normal" 
                          />
                        </div>
                      </div>

                      {/* Bottom Row: Name, Qty, Delete */}
                      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between w-full gap-4 sm:gap-6 mt-1 sm:mt-0">
                        {/* Product Name */}
                        <div className="flex-1 w-full border border-zinc-200/60 rounded-md bg-white focus-within:border-zinc-300 focus-within:ring-2 focus-within:ring-indigo-500/10 transition-all shadow-sm">
                          <Input 
                            placeholder="Item description..." 
                            value={item.name} 
                            onChange={(e) => updateLineItem(index, "name", e.target.value)} 
                            className="w-full h-9 text-sm font-medium text-zinc-900 bg-transparent border-none px-3 shadow-none outline-none focus-visible:ring-0 placeholder:text-zinc-300"
                            disabled={item.type !== "Product"} 
                          />
                        </div>

                        {/* Qty & Trash Stack */}
                        <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-3 shrink-0 sm:pr-4">
                          {/* Qty */}
                          <div className="flex items-center text-xs text-zinc-500 font-medium">
                            ...
                            <div className="relative border border-zinc-200 rounded-md bg-white hover:border-zinc-300 transition-colors shadow-sm">
                              <select 
                                className="h-8 pl-2.5 pr-7 text-xs text-zinc-700 outline-none focus:ring-0 cursor-pointer appearance-none font-medium bg-transparent"
                              ...
                              <ChevronDown className="absolute right-2 top-2.5 size-3 text-zinc-400 pointer-events-none" />
                            </div>
                          </div>
                          ...
                          <Button 
                            ...
                            className="text-red-600 bg-red-50 hover:bg-red-100 size-8 rounded-md transition-all border border-red-100 shadow-sm shrink-0"
                          >
